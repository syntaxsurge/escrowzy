...
export async function broadcastTradeUpdate(
  ...
  const channels = [
    `trade-${trade.id}`, // Specific trade channel
    `user-trades-${trade.buyerId}`, // Buyer's channel
    `user-trades-${trade.sellerId}` // Seller's channel
  ]
  ...
  try {
    ...
    await Promise.all(
      channels.map(channel =>
        pusherServer.trigger(channel, 'trade-updated', eventData)
      )
    )

    // Also broadcast to global channel for admins/dashboard
    if (action === 'created') {
      await pusherServer.trigger('trades-global', 'trade-created', { trade })
    } else {
      await pusherServer.trigger('trades-global', 'trade-updated', { trade })
    }
  } catch (error) {
...
export async function broadcastTradeStatusChange(
  ...
  const channels = [
    `trade-${trade.id}`,
    `user-trades-${trade.buyerId}`,
    `user-trades-${trade.sellerId}`
  ]
  ...
  try {
    await Promise.all(
      channels.map(channel =>
        pusherServer.trigger(channel, 'trade-status-changed', eventData)
      )
    )
  } catch (error) {
    console.error('Failed to broadcast trade status change:', error)
...
export async function broadcastTradeFunded(
  ...
  try {
    await Promise.all(
      channels.map(channel =>
        pusherServer.trigger(channel, 'trade-funded', eventData)
      )
    )
  } catch (error) {
    console.error('Failed to broadcast trade funded event:', error)
...
export async function broadcastTradeCompleted(
  ...
  try {
    await Promise.all(
      channels.map(channel =>
        pusherServer.trigger(channel, 'trade-completed', eventData)
      )
    )
  } catch (error) {
    console.error('Failed to broadcast trade completed event:', error)
...
export async function broadcastTradeDisputed(
  ...
  try {
    await Promise.all(
      channels.map(channel =>
        pusherServer.trigger(channel, 'trade-disputed', eventData)
      )
    )
  } catch (error) {
...
export async function broadcastBattleAccepted(
  ...
  try {
    // Send different events to sender and accepter for proper state transitions
    await Promise.all([
      // Sender receives 'battle-accepted' to transition from invitation-sent state
      pusherServer.trigger(`user-${fromUserId}`, 'battle-accepted', {
        ...battleData,
        timestamp: new Date().toISOString()
      }),
      // Accepter receives 'battle-started' to begin the battle
