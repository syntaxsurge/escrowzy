...
export async function getEscrowById(
  ...
  return {
    ...
    buyer: trade.buyerWallet || '0x0000000000000000000000000000000000000000',
    seller: trade.sellerWallet || '0x0000000000000000000000000000000000000000',
...
export async function listEscrows(filter: EscrowFilter): Promise<{
  ...
  const escrows = results.map(trade => {
    ...
    return {
      ...
      buyer: trade.buyerWallet || '0x0000000000000000000000000000000000000000',
      seller:
        trade.sellerWallet || '0x0000000000000000000000000000000000000000',
      buyerId: trade.buyerId,
      sellerId: trade.sellerId,
      amount: trade.amount,
      currency: trade.currency,
...
export async function createEscrow(data: {
  ...
  return {
    ...
    chainId: trade.chainId,
    buyer:
      buyer[0]?.walletAddress || '0x0000000000000000000000000000000000000000',
    seller:
      seller[0]?.walletAddress || '0x0000000000000000000000000000000000000000',
    buyerId: trade.buyerId,
    sellerId: trade.sellerId,
    amount: trade.amount,
    currency: trade.currency,
    ...
    metadata: trade.metadata,
    contractAddress: process.env.NEXT_PUBLIC_ESCROW_CORE_ADDRESS
  }
}
