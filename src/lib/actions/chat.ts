...
export async function sendMessageAction(
  ...
  if (pusher) {
    ...
    if (contextType === 'direct' && isNewConversation && otherUserId) {
      ...
      await pusher.trigger(`user-${user.id}`, 'new-dm-conversation', {
        user: otherUser
      })

      // Notify the other user about new conversation with the sender
      await pusher.trigger(`user-${otherUserId}`, 'new-dm-conversation', {
        user: {
          id: user.id,
          name: user.name,
...
export async function deleteMessageAction(messageId: number) {
  ...
  if (pusher) {
    await pusher.trigger(
      `chat-${message[0].contextType}-${message[0].contextId}`,
      'message-deleted',
      { messageId }
    )
  }
