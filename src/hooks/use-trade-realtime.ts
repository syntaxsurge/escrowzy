...
export function useTradeRealtime({
  ...
  useEffect(() => {
    ...
    if (tradeId) {
      channels.push(`trade-${tradeId}`)
    }

    // Subscribe to user's trades channel if userId is provided
    if (userId) {
      channels.push(`user-trades-${userId}`)
    }
    ...
    channels.forEach(channelName => {
      ...
      channel.bind('trade-updated', (event: TradeRealtimeEvent) => {
...
export function useAllTradesRealtime(
  ...
  useEffect(() => {
    ...
    const channelName = 'trades-global'
    ...
    channel.bind('trade-created', (event: { trade: TradeWithUsers }) => {
      if (onNewTrade) {
    ...
    // Listen for trade updates
    channel.bind('trade-updated', (event: { trade: TradeWithUsers }) => {
      if (onTradeUpdate) {
        onTradeUpdate(event.trade)
      }
    })
