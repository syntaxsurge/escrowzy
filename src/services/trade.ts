...
export async function disputeTrade(
  ...
  try {
    ...
    metadata.disputeReason = input.description
    if (input.evidence) {
    ...
    await logTradeActivity(trade, `Trade disputed: ${input.reason}`)

    // Broadcast real-time updates
    if (updatedTradeWithUsers) {
      const previousStatus = trade.status as string
      await broadcastTradeDisputed(updatedTradeWithUsers, input.description)
