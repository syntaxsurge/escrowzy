...
export class RewardsService {
  ...
  async updateCombatPower(userId: number, amount: number, reason?: string) {
    const gameData = await this.getOrCreateGameData(userId)
    const newCombatPower = Math.max(0, gameData.combatPower + amount)
   ...
   */
  async handleBattleWin(userId: number, opponentCP: number) {
    const CP_REWARDS = {
      WIN_BATTLE: 15,
      WIN_AGAINST_STRONGER: 30 // Win against higher CP opponent
    }

    const gameData = await this.getOrCreateGameData(userId)
    let cpGain = CP_REWARDS.WIN_BATTLE

    if (opponentCP > gameData.combatPower) {
      cpGain = CP_REWARDS.WIN_AGAINST_STRONGER
    }

    await this.updateCombatPower(userId, cpGain, 'Battle victory')
    await this.addXP(userId, 30, 'Battle won')
  ...
  async handleBattleLoss(userId: number) {
    ...
    await this.addXP(userId, CONSOLATION_XP, 'Battle participation')
