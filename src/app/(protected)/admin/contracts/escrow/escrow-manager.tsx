...
export function EscrowManager() {
      ...
      effectiveChainId,
      {
        onSuccess: () => {
          showSuccessToast(
            ...
            `${amount} ${statsData?.stats?.nativeCurrency} withdrawn`
          )
        }
      }
    )
  ...
  const handleUpdateFee = async () => {
    ...
    await updateFeeButton.execute(async () => {
      await executeTransaction(
        'updateEscrowFee',
        { feePercentage: parseFloat(newFeePercentage) * 100 }, // Convert to basis points
        effectiveChainId,
        {
          onSuccess: () => {
  ...
  const handleUpdateWindow = async () => {
    ...
    await updateWindowButton.execute(async () => {
      await executeTransaction(
        'updateDisputeWindow',
        { days: parseInt(newDisputeWindow) },
        effectiveChainId,
        {
          onSuccess: () => {
            showSuccessToast(
  ...
  const handlePauseContract = async () => {
    ...
    await pauseButton.execute(async () => {
      await executeTransaction(
        isPaused ? 'unpauseEscrow' : 'pauseEscrow',
        {},
        effectiveChainId,
        {
          onSuccess: () => {
